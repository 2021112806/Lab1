...
def run_gbn():
    ...
    from lab_code.gbn_rev import GBN_rev

    ch = input("您想进行upload还是download，请输入选择：")

    if ch == "upload":
        host_1 = GBN_rev(Host.host_address_1, Host.host_address_2)
        host_2 = GBN_rev(Host.host_address_2, Host.host_address_1)
        threading.Thread(target=host_1.client_run).start()  # 注意这里函数一定不能带括号
        threading.Thread(target=host_2.server_run).start()  # 注意这里函数一定不能带括号
...
def run_sr():
    ...
    from lab_code.sr_rev import SR_rev

    ch = input("您想进行upload还是download，请输入选择：")

    if ch == "upload":
        host_1 = SR_rev(Host.host_address_1, Host.host_address_2)
        host_2 = SR_rev(Host.host_address_2, Host.host_address_1)
        threading.Thread(target=host_1.server_run).start()  # 注意这里函数一定不能带括号
        threading.Thread(target=host_2.client_run).start()  # 注意这里函数一定不能带括号
