...
class GBN:
    ...
    def __init__(self, local_address=Host.host_address_1, remote_address=Host.host_address_2):
        ...
        self.data = []  # 缓存发送数据
        self.read_path = '../file/read_download_file.txt'  # 需要发送的源文件数据
        self.ack_buf_size = 10
        self.get_data_from_file()

        self.data_buf_size = 1678  # 作为客户端接收数据缓存
        self.exp_seq = 0  # 当前期望收到该序号的数据
        self.save_path = '../file/save_download_file.txt'  # 接收数据时，保存数据的地址
        self.save_path_ack = '../file/save_download_file_GBN_ack.txt'  # 接收ack时，保存ack信息
