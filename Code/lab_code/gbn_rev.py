...
class GBN_rev:
    ...
    def __init__(self, local_address=Host.host_address_1, remote_address=Host.host_address_2):
        ...
        self.data = []  # 缓存发送数据
        self.read_path = '../file/read_upload_file.txt'  # 需要发送的源文件数据
        self.ack_buf_size = 10
        self.get_data_from_file()

        self.data_buf_size = 1678  # 作为服务端接收数据缓存
        self.exp_seq = 0  # 当前期望收到该序号的数据
        self.save_path = '../file/save_upload_file.txt'  # 接收数据时，保存数据的地址
        self.save_path_ack = '../file/save_upload_file_GBN_ack.txt'  # 接收ack时，保存ack信息
        ...
        if self.next_seq - self.send_base < self.window_size:  # 窗口中仍有可用空间
            if random.random() > self.pkt_loss:  # 随机产生丢包行为
                self.socket.sendto(Host.make_pkt(self.next_seq, self.data[self.next_seq]),
                                   self.remote_address)
            print('客户端:成功发送数据' + str(self.next_seq))
            self.next_seq = self.next_seq + 1
        else:  # 窗口中无可用空间
            print('客户端：窗口已满，暂不发送数据')
    ...
    def client_run(self):
        while True:
            ...
            if len(readable) > 0:  # 接收ACK数据
                ...
                print('收到服务器ACK:' + rcv_ack)
                self.write_data_to_file_ack('\n收到服务器ACK:' + rcv_ack)
    ...
    def write_data_to_file_ack(self, data, mode='a'):
        with open(self.save_path_ack, mode, encoding='utf-8') as f:
            f.write(data)  # 模拟将数据交付到上层
