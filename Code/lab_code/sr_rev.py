...
class SR_rev:
    ...
    def __init__(self, local_address=Host.host_address_1, remote_address=Host.host_address_2):
        ...
        self.data_buf_size = 1678  # 作为服务器接收数据缓存
        self.save_path = '../file/save_upload_file.txt'  # 接收数据时，保存数据的地址
        self.save_path_ack = '../file/save_upload_file_SR_ack.txt'  # 接收ack时，保存ack信息
    ...
    def send_data(self):
        ...
        if self.next_seq < self.send_base + self.send_window_size:  # 窗口中仍有可用空间
            ...
            print('客户端:发送数据' + str(self.next_seq))
            self.next_seq += 1
        else:  # 窗口中无可用空间
    ...
    def slide_rcv_window(self):
        while self.rcv_data.get(self.rcv_base) is not None:  # 循环直到出现未接受的数据包
            ...
            print('服务器:窗口滑动到' + str(self.rcv_base))
